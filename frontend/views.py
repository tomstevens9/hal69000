...
def upload_sound(request):
    ...
    return Response('Failed to upload sound', status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_sound(request, command):
    ...
    return Response(sound.filename.read(),
                    content_type=mime.from_file(str(sound.filename.file)))
